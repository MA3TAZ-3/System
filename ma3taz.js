...
client.on("message", msg => {
  ...
  if (msg.content.includes("@everyone")) {
    ...
    msg.delete();
    msg.reply("```You cant send everyone``` <a:ma3taz_23:799630647513317387>");
  }
});
/////////////
client.on("message", msg => {
  ...
  if (msg.content.includes("@here")) {
    ...
    if (!msg.channel.guild) return;
    msg.delete();
    msg.reply("```You cant send here ``` <a:ma3taz_23:799630647513317387>");
  }
});
//////////////
client.on("message", message => {
...
client.on("message", message => {
  if (message.content.includes("discord.gg")) {
    if (!message.member.hasPermission("MANAGE_MESSAGE")) {
      message.delete();
      message.reply("```you can send partner``` <a:ma3taz_11:799293444136108084>");
      message.react("🚫");
    }
  }
});
...
///////////////////////////////////////////////
   client.on('message', message => {
    ...
    if (!user) return message.channel.send('**id A member User**');
      message.channel.send(`<a:ma3taz_28:840493514244751400>** [ ${user.username} ] Id :**<a:jano_28:840493514244751400>`);
      message.channel.send(`${user.id}`);
}
}); 
   ///////////////////////////
   client.on("message", MA3TAZ => {
    if (MA3TAZ.content.startsWith(prefix + "avatar")) {
  var embed = new Discord.MessageEmbed()
         .setAuthor(`${MA3TAZ.author.username}`, MA3TAZ.author.avatarURL({dynamic: true}))
         .setColor('#0000ff')
         .setDescription(`**[Avatar Link](${MA3TAZ.author.avatarURL({dynamic: true, format: 'png', size: 1024})})**`)
         .setImage(MA3TAZ.author.avatarURL({dynamic: true, format: 'png', size: 1024}))
         .setFooter(`Requsted by ${JANO.author.tag}`, MA3TAZ.author.avatarURL({dynamic: true}))
    MA3TAZ.channel.send(embed);
}
});
...
client.on("message", message => {
  ...
  if (commands[0] == prefix + "say") {
    ...
    if (!message.guild.member(message.author).hasPermission("ADMINISTRATOR"))
      return message.reply("**You Dont Have `ADMINISTRATOR` Permission ** <a:ma3taz_11:799293444136108084>");
    if (!message.guild.member(client.user).hasPermission("ADMINISTRATOR"))
      return message.reply(
        "Please Check My Role Permission To `ADMINISTRATOR <a:ma3taz_11:799293444136108084>"
      );
